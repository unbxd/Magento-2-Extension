 ...
 */
class ReindexJobId extends Column
{
    /**
     * @var FilterManager
     */
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as &$item) {
                $reindexJobId = $item['reindex_job_id'];
                if ($reindexJobId) {
                    $item['reindex_job_id'] = $this->formatValue($reindexJobId);
                }
    ...
    {
        $viewDetailsUrl = $this->getUrl('unbxd_productfeed/indexing_queue/viewDetails', ['id' => $id]);
